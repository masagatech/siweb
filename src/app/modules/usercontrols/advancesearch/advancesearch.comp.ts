...
export class AdvanceSearchComponent implements OnInit, OnDestroy {
    ...
    ddlSearches: any = '';
    isFilterOn = false;
    currentGridView: any = null; //10719
    ...
    ngOnInit(): void {
        ...
        if (this.currentGridView == this.global.getGridControlName()) {
            let globalFilteredData = this.global.getSaveGridFilter();
            this.filterData = globalFilteredData[0] || [];
            this.isFilterOn = globalFilteredData[1] || false;
        }
    }
   ...
   */
    ...
    clear() {
        ...
        this.global.setSaveGridFilter(null, this.isFilterOn, null); //10719
    }
    ...
    search() {
        ...
        this.global.setSaveGridFilter(arr, this.isFilterOn, this.currentGridView); //10719
    }
    ...
    removeSavedFilter() {
        this.filterService.removeSavedSearch({
            'userid': this.global.getUser().id,
            'id': this.ddlSearches
        }).subscribe((res: any) => {
            if (res.resultKey === 1) {
                this.dbTableData.id = '';
                this.dbTableData.filterName = '';
                this.dbTableData.active = true;
                this.dbTableData.isdelete = false;
                this.dbTableData.module = this.controlName;
                this.filterData = [];
                this.searchList=this.searchList.filter((a) => {
                    return a.id != this.ddlSearches;
                });
                this.ddlSearches = '';
            }
        });
    ...
    selectSavedSearch(event, iscall) {
        ...
        this.filterService.getSavedSearch({
            ...
            if (res.resultKey === 1) {
                let selectedSearch = res.resultValue;
